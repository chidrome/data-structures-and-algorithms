...
const splitFoods = (recipe) => {
  ...
  const ingredients = recipe.ingredients;
  for(let i = 0; i < ingredients.length; i++){
    let arrayToSplit = ingredients[i].split(' ');
    arrayToSplit.splice(0, 2);
    let ingredientToBuy = arrayToSplit.join(' ');
    result.push(ingredientToBuy)
  }
  return result;
};
